...
if (
	...
	if (mix.inProduction()) {
		...
		fs.ensureDir(languages, function (err) {
			...
			wpPot({
				package: "Radius Demo Importer",
				bugReport: "",
				src: "**/*.php",
				domain: "radius-demo-importer",
				destFile: "languages/radius-demo-importer.pot",
			});
		});
	}
	 ...
	 * JS
	 */
	mix.js('src/js/admin.js', 'assets/js/admin.min.js');

	/**
	 * CSS
	 */
	if (!mix.inProduction()) {
		mix.sass("src/scss/admin.scss", "assets/css/admin.min.css",).sourceMaps(true, 'source-map');
	} else {
